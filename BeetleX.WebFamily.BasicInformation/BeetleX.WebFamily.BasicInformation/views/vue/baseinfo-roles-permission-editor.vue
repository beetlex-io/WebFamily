<div>
            ...
            <li v-for="(item,i) in items">
                ...
                <el-table v-if="item.extend" size="mini" :show-header="i==0" :data="item.Items" style="margin-left:20px;width:550px;">
                    ...
                    <el-table-column label="是否可用" align="right" header-align="right">
                        <template slot-scope="item">
                            <el-switch size="mini" v-model="item.row.Value"></el-switch>
                        </template>
                    </el-table-column>
                </el-table>
            </li>
    ...
    <el-row>
        <el-col span="24" style="text-align:right;padding-top:10px;">
            ...
            <el-button size="mini" style="padding-left:10px; padding-right:10px;" @click="onModify">确定</el-button>
        </el-col>
    </el-row>
</div>
<script>
    export default {
        ...
        methods: {
            onGet(id) {
                this.roleID = id;
                this.$get('/baseinfo/roles/ListPermission', { id: id }).then(r => {
                    r.forEach(v => {
                        v.extend = true;
                    });
                    this.items = r;
            ...
            onModify() {
                this.$confirmMsg('是否保存角色权限配置?', () => {
                    this.$post('/baseinfo/roles/ModifyPermission', { id: this.roleID, items: this.items }).then(r => {
                        this.$emit('close');
                    });
                });
            }
